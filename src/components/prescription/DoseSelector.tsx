import { useState, useEffect } from "react";
import { Check, ChevronsUpDown } from "lucide-react";
import { Command, CommandEmpty, CommandGroup, CommandInput, CommandItem, CommandList } from "@/components/ui/command";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { Button } from "@/components/ui/button";
import { supabase } from "@/integrations/supabase/client";
import { cn } from "@/lib/utils";

interface DosageForm {
  id: number;
  name: string;
}

interface DoseSelectorProps {
  value: string;
  onChange: (dose: string) => void;
}

const tabletDoses = [
  "১+০+১ (খাবারের পরে)",
  "১+১+১ (খাবারের পরে)",
  "০+০+১ (খাবারের পরে)",
  "১+০+০ (খাবারের পরে)",
  "০+১+০ (খাবারের পরে)",
  "১+১+০ (খাবারের পরে)",
  "০+১+১ (খাবারের পরে)",
  "১+০+১ (খাবারের আগে)",
  "১+১+১ (খাবারের আগে)",
  "০+০+১ (খাবারের আগে)",
  "১+০+০ (খাবারের আগে)",
  "০+১+০ (খাবারের আগে)",
  "১+১+০ (খাবারের আগে)",
  "০+১+১ (খাবারের আগে)",
  "১/২+০+১/২ (খাবারের পরে)",
  "১/২+১/২+১/২ (খাবারের পরে)",
  "১/৪+০+১/৪ (খাবারের পরে)",
  "১/৪+১/৪+১/৪ (খাবারের পরে)",
  "২+০+২ (খাবারের পরে)",
  "২+২+২ (খাবারের পরে)",
  "১+০+০+১ (খাবারের পরে)",
  "১+১+১+১ (খাবারের পরে)",
  "০+০+১+০ (খাবারের পরে)",
  "দিনে ১ বার সকালে খালি পেটে",
  "দিনে ১ বার রাতে খাবারের পরে",
  "৬ ঘন্টা পর পর",
  "৮ ঘন্টা পর পর",
  "১২ ঘন্টা পর পর",
  "প্রতি ৪ ঘন্টায় ১টি",
];

const syrupDoses = [
  "১ চা চামচ দিনে ২ বার (খাবারের পরে)",
  "১ চা চামচ দিনে ৩ বার (খাবারের পরে)",
  "২ চা চামচ দিনে ২ বার (খাবারের পরে)",
  "২ চা চামচ দিনে ৩ বার (খাবারের পরে)",
  "১/২ চা চামচ দিনে ২ বার",
  "১/২ চা চামচ দিনে ৩ বার",
  "৫ মিলি দিনে ২ বার (খাবারের পরে)",
  "৫ মিলি দিনে ৩ বার (খাবারের পরে)",
  "১০ মিলি দিনে ২ বার (খাবারের পরে)",
  "১০ মিলি দিনে ৩ বার (খাবারের পরে)",
  "১৫ মিলি দিনে ২ বার",
  "২.৫ মিলি দিনে ২ বার",
  "২.৫ মিলি দিনে ৩ বার",
  "১ চা চামচ দিনে ১ বার (রাতে)",
  "২ চা চামচ দিনে ১ বার (রাতে)",
  "১ চা চামচ দিনে ১ বার (সকালে)",
  "প্রতি ৬ ঘন্টায় ১ চা চামচ",
  "প্রতি ৮ ঘন্টায় ১ চা চামচ",
];

const dropDoses = [
  "১ ড্রপ প্রতি চোখে দিনে ২ বার",
  "১ ড্রপ প্রতি চোখে দিনে ৩ বার",
  "১ ড্রপ প্রতি চোখে দিনে ৪ বার",
  "২ ড্রপ প্রতি চোখে দিনে ২ বার",
  "২ ড্রপ প্রতি চোখে দিনে ৩ বার",
  "২ ড্রপ প্রতি চোখে দিনে ৪ বার",
  "১ ড্রপ প্রতি চোখে প্রতি ২ ঘন্টায়",
  "১ ড্রপ প্রতি চোখে রাতে ঘুমানোর আগে",
  "১ ড্রপ প্রতি কানে দিনে ২ বার",
  "২ ড্রপ প্রতি কানে দিনে ২ বার",
  "২ ড্রপ প্রতি কানে দিনে ৩ বার",
  "৩ ড্রপ প্রতি কানে দিনে ২ বার",
  "১ ড্রপ প্রতি নাকে দিনে ২ বার",
  "২ ড্রপ প্রতি নাকে দিনে ২ বার",
  "২ ড্রপ প্রতি নাকে দিনে ৩ বার",
  "৩ ড্রপ প্রতি নাকে দিনে ২ বার",
];

const injectionDoses = [
  "১ অ্যাম্পুল আই.এম দিনে ১ বার",
  "১ অ্যাম্পুল আই.এম দিনে ২ বার",
  "১ অ্যাম্পুল আই.ভি দিনে ১ বার",
  "১ অ্যাম্পুল আই.ভি দিনে ২ বার",
  "১ অ্যাম্পুল আই.ভি দিনে ৩ বার",
  "১ ভায়াল আই.ভি দিনে ১ বার",
  "১ ভায়াল আই.ভি দিনে ২ বার",
  "১ ভায়াল আই.ভি দিনে ৩ বার",
  "১ ভায়াল আই.এম দিনে ১ বার",
  "সাবকিউটেনিয়াস প্রয়োজন অনুযায়ী",
  "সাবকিউটেনিয়াস দিনে ১ বার",
  "সাবকিউটেনিয়াস সপ্তাহে ১ বার",
  "আই.ভি ইনফিউশন ৮ ঘন্টা পর পর",
  "আই.ভি ইনফিউশন ১২ ঘন্টা পর পর",
  "আই.ভি বোলাস প্রয়োজন অনুযায়ী",
];

const creamDoses = [
  "আক্রান্ত স্থানে দিনে ২ বার লাগাতে হবে",
  "আক্রান্ত স্থানে দিনে ৩ বার লাগাতে হবে",
  "আক্রান্ত স্থানে দিনে ১ বার লাগাতে হবে",
  "আক্রান্ত স্থানে দিনে ৪ বার লাগাতে হবে",
  "পাতলা করে দিনে ২ বার লাগাতে হবে",
  "পাতলা করে দিনে ৩ বার লাগাতে হবে",
  "প্রয়োজন অনুযায়ী লাগাতে হবে",
  "রাতে ঘুমানোর আগে লাগাতে হবে",
  "সকালে ও রাতে লাগাতে হবে",
  "স্নানের পরে দিনে ২ বার",
];

const inhalerDoses = [
  "২ পাফ দিনে ২ বার",
  "২ পাফ দিনে ৩ বার",
  "১ পাফ দিনে ২ বার",
  "১ পাফ দিনে ৩ বার",
  "১ পাফ দিনে ৪ বার",
  "২ পাফ দিনে ৪ বার",
  "১ পাফ প্রতি ৬ ঘন্টায়",
  "২ পাফ প্রয়োজন অনুযায়ী",
  "প্রয়োজন অনুযায়ী (সর্বোচ্চ ৮ পাফ/দিন)",
];

const nebuliserDoses = [
  "১ অ্যাম্পুল নেবুলাইজার দিনে ২ বার",
  "১ অ্যাম্পুল নেবুলাইজার দিনে ৩ বার",
  "১ অ্যাম্পুল নেবুলাইজার ৬ ঘন্টা পর পর",
  "১ অ্যাম্পুল নেবুলাইজার ৮ ঘন্টা পর পর",
  "২ মিলি নেবুলাইজার দিনে ২ বার",
  "২ মিলি নেবুলাইজার দিনে ৩ বার",
  "নেবুলাইজার প্রয়োজন অনুযায়ী",
];

const suppositoryDoses = [
  "১টি রেক্টাল দিনে ১ বার",
  "১টি রেক্টাল দিনে ২ বার",
  "১টি রেক্টাল প্রয়োজন অনুযায়ী",
  "১টি রেক্টাল জ্বর হলে",
  "১টি ভ্যাজাইনাল দিনে ১ বার রাতে",
  "১টি ভ্যাজাইনাল দিনে ২ বার",
];

const sprayDoses = [
  "১ স্প্রে প্রতি নাকে দিনে ২ বার",
  "২ স্প্রে প্রতি নাকে দিনে ২ বার",
  "১ স্প্রে প্রতি নাকে দিনে ৩ বার",
  "২ স্প্রে প্রতি নাকে দিনে ১ বার",
  "১ স্প্রে মুখে দিনে ২ বার",
  "২ স্প্রে মুখে দিনে ৩ বার",
];

const patchDoses = [
  "১টি প্যাচ দিনে ১ বার",
  "১টি প্যাচ প্রতি ২৪ ঘন্টায় পরিবর্তন করতে হবে",
  "১টি প্যাচ সপ্তাহে ১ বার",
  "১টি প্যাচ সপ্তাহে ২ বার",
];

const sachetsAndPowderDoses = [
  "১ স্যাশে দিনে ১ বার",
  "১ স্যাশে দিনে ২ বার",
  "১ স্যাশে দিনে ৩ বার",
  "১ স্যাশে রাতে ঘুমানোর আগে",
  "১ প্যাকেট দিনে ১ বার",
  "১ প্যাকেট দিনে ২ বার",
  "১ প্যাকেট দিনে ৩ বার",
];

const insulinDoses = [
  "খাবারের আগে রক্তের শর্করা অনুযায়ী",
  "সকালে __ ইউনিট, রাতে __ ইউনিট",
  "দিনে ১ বার __ ইউনিট",
  "দিনে ২ বার __ ইউনিট",
  "সকালে __ ইউনিট",
  "রাতে __ ইউনিট",
];

const generalDoses = [
  "দিনে ১ বার (সকালে)",
  "দিনে ১ বার (রাতে)",
  "দিনে ২ বার (সকাল ও রাত)",
  "দিনে ৩ বার",
  "দিনে ৪ বার",
  "সপ্তাহে ১ বার",
  "সপ্তাহে ২ বার",
  "সপ্তাহে ৩ বার",
  "মাসে ১ বার",
  "প্রতি ২ সপ্তাহে ১ বার",
  "প্রয়োজন অনুযায়ী",
  "খালি পেটে",
  "খাবারের সাথে",
  "ঘুমানোর আগে",
  "সকালে খালি পেটে",
  "খাবারের ৩০ মিনিট আগে",
  "খাবারের ১ ঘন্টা পরে",
];

const DoseSelector = ({ value, onChange }: DoseSelectorProps) => {
  const [open, setOpen] = useState(false);
  const [dosageForms, setDosageForms] = useState<DosageForm[]>([]);
  const [customDose, setCustomDose] = useState("");

  useEffect(() => {
    const fetchDosageForms = async () => {
      const { data } = await supabase
        .from("dosage_forms")
        .select("id, name")
        .order("name")
        .limit(20);
      
      if (data) setDosageForms(data);
    };
    
    fetchDosageForms();
  }, []);

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          role="combobox"
          aria-expanded={open}
          className="w-full justify-between font-normal text-left h-auto py-2 px-3"
          style={{ 
            fontSize: "14px",
            fontWeight: value ? 600 : 400,
            fontStyle: value ? "normal" : "italic",
            color: value ? "#000" : "#999",
            paddingLeft: "15px"
          }}
        >
          {value || "→ ডোজ: Select dose..."}
          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-[450px] p-0 bg-white dark:bg-gray-800 z-[9999] shadow-lg border" align="start">
        <Command className="bg-white dark:bg-gray-800">
          <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-blue-50 dark:bg-blue-900/20">
            <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">
              📝 কাস্টম ডোজ লিখুন
            </label>
            <input
              type="text"
              placeholder="যেমনঃ ১+০+১ (খাবারের পরে ৭ দিন)"
              className="w-full px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              value={customDose}
              onChange={(e) => setCustomDose(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' && customDose.trim()) {
                  onChange(`→ ডোজ: ${customDose.trim()}`);
                  setCustomDose("");
                  setOpen(false);
                }
              }}
            />
            {customDose.trim() && (
              <Button
                size="sm"
                className="w-full mt-2 bg-blue-600 hover:bg-blue-700 text-white"
                onClick={() => {
                  onChange(`→ ডোজ: ${customDose.trim()}`);
                  setCustomDose("");
                  setOpen(false);
                }}
              >
                ✓ এই ডোজ ব্যবহার করুন
              </Button>
            )}
          </div>
          <CommandInput placeholder="ডোজ খুঁজুন..." className="border-b" />
          <CommandList className="max-h-[400px]">
            <CommandEmpty>কোনো ডোজ পাওয়া যায়নি</CommandEmpty>
            
            <CommandGroup heading="💊 ট্যাবলেট/ক্যাপসুল">
              {tabletDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="🥄 সিরাপ">
              {syrupDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="💧 ড্রপ (চোখ/কান/নাক)">
              {dropDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="💉 ইনজেকশন/আই.ভি">
              {injectionDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="🧴 ক্রিম/অয়েন্টমেন্ট">
              {creamDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="🌬️ ইনহেলার">
              {inhalerDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="🌫️ নেবুলাইজার">
              {nebuliserDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="🔺 সাপোজিটরি">
              {suppositoryDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="💨 স্প্রে">
              {sprayDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="📋 প্যাচ">
              {patchDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="📦 স্যাশে/পাউডার">
              {sachetsAndPowderDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="💉 ইনসুলিন">
              {insulinDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>

            <CommandGroup heading="⏰ সাধারণ">
              {generalDoses.map((dose) => (
                <CommandItem
                  key={dose}
                  value={dose}
                  onSelect={() => {
                    onChange(`→ ডোজ: ${dose}`);
                    setOpen(false);
                  }}
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value.includes(dose) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {dose}
                </CommandItem>
              ))}
            </CommandGroup>
          </CommandList>
        </Command>
      </PopoverContent>
    </Popover>
  );
};

export default DoseSelector;
